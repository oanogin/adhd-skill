...
4. **Configure `.gitignore`.** Append `.superpowers/` and `project/repos.local.json`
   to `.gitignore` (create the file if it does not exist). Do NOT gitignore
   `.impeccable/` — it is tracked. Do NOT gitignore `project/` itself — it holds
   tracked, durable project state and must be committed; only the single file
   `project/repos.local.json` (per-user repo→path bindings) is ignored.
...
- `.gitignore` — extended with `.superpowers/` and `project/repos.local.json`
  (`.impeccable/` stays tracked; `project/` itself stays tracked).
...
`setup` always scaffolds in `single` mode with `colocated` prototype topology —
`state.json` is created with `mode: "single"`, an empty `repos` registry, and
`prototypeTopology: "colocated"`. For a multi-repo product, or one whose prototype app
is standalone (its own app or a separate repo from production), run the `workspace`
command after `setup` to switch to `multi` mode, register code repos, and set the
prototype topology; for an existing project, run `adopt` instead of the front-load
stages.
