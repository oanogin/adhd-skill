...
**Effort:** medium
**Gate:** `project/milestones/m{{N}}/tracer.md` exists (Tracer done); production-track
milestones only.
**Output:** an updated `project/milestones/m{{N}}/overview.md`, an updated prototype
where the tracer contradicted it, and an updated `project/milestones/m{{N}}/tracer.md`.
**Sub-skill:** none.
...
user to run `{{command_prefix}}adhd tracer --milestone {{N}}` first. `replan` runs only
on production-track milestones.
...
1. **Revise the surface plan against the tracer.** Re-read the tracer findings and the
   ...
   what the tracer actually discovered — add surfaces the reality made necessary, remove
   ones that no longer make sense, reorder them, and rescope where the tracer proved the
   original estimate wrong. If a surface must change, re-run `design` for it.
2. **Reconcile the prototype.** Where the tracer's backend reality contradicts the
   signed-off prototype, the prototype is updated FIRST — it stays the current
   reference. Update the affected prototype surfaces (via `design`), so the prototype
   the `gap` stage compares against is already correct.
...
4. `node {{scripts_path}}/context-watch.mjs --next gap` — if it advises a fresh
...
6. Tell the user the next runnable stage is `gap` for milestone {{N}}.
