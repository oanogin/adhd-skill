...
2. **Review the milestone's surfaces.** Check each surface against
...
3. **Check story and feature coverage.** Verify every story chosen in `m{{N}}/brief.md`
...
4. **Write findings to `m{{N}}/review.md`.** Record each as an actionable item: the
   issue, the surface or feature it affects, a severity, and the fix.
5. **File and fix defects before closing.** Defects found in review are fixed before
